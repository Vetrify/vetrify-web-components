 ...
 * @status stable
 *
 * @slot trigger - The dropdown's trigger, usually a `<v-button>` element.
...
export class Dropdown {
  ...
  @Element() host: HTMLVDropdownElement;
  ...
  async show() {
    ...
    this.panel.addEventListener('v-activate', this.handleMenuItemActivate);
    this.panel.addEventListener('v-select', this.handlePanelSelect);
  ...
  async hide() {
    ...
    this.panel.removeEventListener('v-activate', this.handleMenuItemActivate);
    this.panel.removeEventListener('v-select', this.handlePanelSelect);
    document.addEventListener('keydown', this.handleDocumentKeyDown);
  ...
  getMenu() {
    return this.panel
      ...
      .filter(el => el.tagName.toLowerCase() === 'v-menu')[0] as HTMLVMenuElement;
  }
  ...
  handleDocumentKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === 'Tab') {
      ...
      if (this.open && document.activeElement?.tagName.toLowerCase() === 'v-menu-item') {
  ...
  handleMenuItemActivate(event: CustomEvent) {
    const item = event.target as HTMLVMenuItemElement;
  ...
  handlePanelSelect(event: CustomEvent) {
    ...
    if (this.closeOnSelect && target.tagName.toLowerCase() === 'v-menu') {
      this.hide();
      this.focusOnTrigger();
    }
  }
  ...
  handleTriggerKeyDown(event: KeyboardEvent) {
    ...
    const menuItems = menu ? [...menu.querySelectorAll('v-menu-item')] : null;
  ...
  //
  // For example, the accessible trigger of an <v-button> is a <button> located inside its shadow root.
  //
