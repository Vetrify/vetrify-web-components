...
export class Form {
   ...
   */
  @Event({ eventName: 'v-submit' }) vSubmit: EventEmitter<{ formData: FormData; formControls: HTMLElement[] }>;
  ...
  connectedCallback() {
    this.formControls = [
      ...
      {
        tag: 'v-range',
        serialize: (el: HTMLVRangeElement, formData) => {
          if (el.name && !el.disabled) {
            formData.append(el.name, el.value + '');
          }
        },
      ...
      {
        tag: 'v-select',
        serialize: (el: HTMLVSelectElement, formData) => {
          if (el.name && !el.disabled) {
            if (el.multiple) {
  ...
  async submit() {
    ...
    this.vSubmit.emit({ formData, formControls });
