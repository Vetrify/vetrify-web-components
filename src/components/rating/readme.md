...
| `getSymbol` | --          | A function that returns the symbols to display. Accepts an option `value` parameter you can use to map a specific symbol to a value. | `(value?: number) => string` | `(value?: number) => '<v-icon name="star-fill"></v-icon>'` |
...
| ---------- | ---------------------------------------- | ------------------ |
| `v-change` | Emitted when the rating's value changes. | `CustomEvent<any>` |
