...
export class TabGroup {
  ...
  @Element() host: HTMLVTabGroupElement;
  ...
  handleClick(event: MouseEvent) {
    ...
    const tab = target.closest('v-tab');
    const tabGroup = tab?.closest('v-tab-group');
  ...
  handleKeyDown(event: KeyboardEvent) {
    ...
    if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
      ...
      if (activeEl && activeEl.tagName.toLowerCase() === 'v-tab') {
        const tabs = this.getAllTabs();
  ...
  setActiveTab(tab: HTMLVTabElement, emitEvents = true) {
